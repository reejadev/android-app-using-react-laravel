...
<View className="items-center px-5 pt-10">
  ...
  <Text className={`text-[28px] 
    font-bold text-center mb-3 ${currentTheme === 'dark' ? 'text-white' : 'text-gray-900'  }`}>
Imaginary
</Text>


<Text className={`text-base text-center mb-10 leading-6 ${currentTheme === 'dark' ?
   'text-gray-300' : 'text-gray-600'}`}> 
   Transform your images with powerful AI tools - 
  recolor, restore, fill, and remove objects with just a few taps</Text>

  <View className="flex-row flex-wrap justify-between mb-[30px] px-[5px]">{features.map ((feature, index)=>
  (<View key={index}
...
<TouchableOpacity className="h-[54px] rounded-xl border-[1.5px] 
justify-center items-center mb-4" style={{ borderColor: colors.primary}}
...
style={{ height: 54, borderRadius: 12, marginBottom: 16 }}

>
<TouchableOpacity className="h-[54px] justify-center items-center"
onPress={() => router.push('/signup')}>

  <Text className="text-base font-semibold text-white" style={{color:colors.primaryDark}}>Create Account</Text>
</TouchableOpacity>

</LinearGradient>

<Text className={`text-center text-sm mt-2 ${currentTheme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Start transforming your images today</Text>
